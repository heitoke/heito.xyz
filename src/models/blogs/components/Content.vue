                    ...
                    >
                        <div class="image" v-for="image of block?.images" :key="image">
                            ...
                            <ul class="buttons" v-if="edit">
                                <li class="blur" @mouseenter="setToolpic({ text: 'Change image' })"
                                ...
                                <li class="blur" @mouseenter="setToolpic({ text: 'Remove image' })"
                                    @click="update(content => {
                                        const i = content[rowId][listId].childrens[childrenId][blockId]?.images?.findIndex(i => image === i);
                        ...
                        <AnimationHeight :showed="edit">
                            <ul class="buttons">
                                <li v-for="btn of buttons" :key="btn.icon"
                                    @mouseenter="setToolpic({ text: btn.label })"
...
import type { IBlogContent, IBlogContentChildren } from '../../../libs/api/routes/blogs';
...
export default defineComponent({
            ...
            return [
                {
                    ...
                    click: (rowId: number, listId: number, childrenId: number, blockId: number) => {
                        this.update(content => {
                            this.textWindow('New image', 'image', 'Image (URL)', url => {
                ...
                },
                {
                    ...
                    click: (rowId: number, listId: number, childrenId: number, blockId: number) => {
                        this.update(content => {
                            this.textWindow('Change size', 'layers', 'Number', num => {
    ...
    methods: {
        ...
        getRowColumns(row: Array<IBlogContent> | Array<IBlogContentChildren>) {
            let n = 0;
        ...
        update(callback: (content: TContent) => void) {
            let content = [...this.content || []];

            callback(content as any);

            this.$emit('update', content as any);
        },
        textWindow(title: string, icon: string, label: string, callback: (text: string) => void) {
            let text = '';
