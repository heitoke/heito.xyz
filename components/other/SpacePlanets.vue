<template>
    <div class="space-planets" ref="root">
        <ul class="buttons blur" :style="buttonsOptions?.style || ''">
            <li :class="{ rotate: options.buttons }"
                @mouseenter="$toolpics.set({ text: (options.buttons ? 'Hide' : 'Show') + ' buttons', position: 'left' })"
                @click="options.buttons = !options.buttons"
            >
                <Icon name="arrow-down"/>
            </li>

            <Height :showed="options.buttons">
                <li v-for="btn of getListButtonsOptions" :key="btn.icon"
                    :class="{ active: btn?.active }"
                    @mouseenter="$toolpics.set({ text: btn.label, position: 'left' })"
...
const props = defineProps({
    parent: {
        type: Object as PropType<HTMLElement | Element>
    },
    buttonsOptions: { type: Object as PropType<PropButtonsOptions> }
});
...
const getListButtonsOptions = computed<Array<OptionButton>>(() => {
    return [
        {
            label: 'Rotation',
            icon: 'damage-void',
            active: options.value.rotation,
            click: () => {
                options.value.rotation = !options.value.rotation;
            }
        },
        {
            label: 'Move',
            icon: 'hand',
            active: options.value.move,
            click: () => {
                options.value.move = !options.value.move;
            }
        },
        {
            label: 'Fullscreen',
            icon: 'cube',
            active: options.value.fullscreen,
            click: () => {
                ...
                if (options.value.fullscreen) {
                    ...
                    options.value.fullscreen = false;
                } else {
...
onMounted(async () => {
    const [listUsers, status] = await $api.users.list({ limit: 500, fields: ['_id', 'color'] });
    ...
    if (root?.value) root?.value?.appendChild(renderer.domElement);
});
...
window.addEventListener('pointermove', (event: MouseEvent) => {
    ...
    if (intersects.length < 1) {
        if (root.value) root.value['style']['cursor'] = 'default';
        return;
    }
    ...
    if (root.value) root['value']!['style']['cursor'] = 'pointer';
});
...
window.addEventListener('pointerdown', (event: MouseEvent) => {
    ...
    focusObj['cb'] = async () => {
        let [resultUser, status] = await $api.users.get(userData?._id);
        ...
        root.value!['style']['cursor'] = 'default';
    }
...
watch(() => props.parent?.clientWidth, (e) => {
    console.log(e);
...
function animate() {
    ...
    if (!profileOpen.value && options.value.move) controls.update(delta);
